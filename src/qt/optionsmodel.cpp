...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeTaliumAmount"))
        settings.setValue("nAnonymizeTaliumAmount", 1000);
    nAnonymizeTaliumAmount = settings.value("nAnonymizeTaliumAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeTaliumAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeTaliumAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeTaliumAmount:
            return QVariant(nAnonymizeTaliumAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeTaliumAmount:
            nAnonymizeTaliumAmount = value.toInt();
            settings.setValue("nAnonymizeTaliumAmount", nAnonymizeTaliumAmount);
            emit AnonymizeTaliumAmountChanged(nAnonymizeTaliumAmount);
            break;
        default:
            break;
        }
