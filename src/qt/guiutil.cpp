...
{
    // Convert Talium:// to Talium:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("Talium://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "Talium:");
    }
...
void openConfigfile()
{
    ...
    /* Open mastercoin.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Talium.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Talium.lnk
...
{
    return GetAutostartDir() / "Talium.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a Talium.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Talium\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the Talium app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add Talium app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Talium-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  Talium-qt [" + tr("command-line options") + "]                     " + "\n";
