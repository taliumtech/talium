...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeTaliumAmount = BitcoinUnits::formatWithUnit(false, nAnonymizeTaliumAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeTaliumAmount = strAnonymizeTaliumAmount.remove(strAnonymizeTaliumAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeTaliumAmount + " TLM / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeTaliumAmount*COIN) nMaxToAnonymize = nAnonymizeTaliumAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeTaliumAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeTaliumAmount));
        strAnonymizeTaliumAmount = strAnonymizeTaliumAmount.remove(strAnonymizeTaliumAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeTaliumAmount + " TLM / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
