...
{
	resize(900, 520);
	setWindowTitle(tr("Talium") + " - " + tr("Wallet"));
#ifndef Q_OS_MAC
...
#endif
	setObjectName("Talium");
	setStyleSheet("#Talium { background-color: qradialgradient(cx: -0.8, cy: 0, fx: -0.8, fy: 0, radius: 1.4, stop: 0 #ffffff, stop: 1 #ffffff);  }");
	// Accept D&D of URIs
	setAcceptDrops(true);
...
void BitcoinGUI::createActions()
{
	...
	sendCoinsAction->setToolTip(tr("Send coins to a Talium address"));
	...
	optionsAction->setToolTip(tr("Modify configuration options for Talium"));
...
void BitcoinGUI::createToolBars()
{
	...
	toolbar->setStyleSheet("QToolButton { color: #000000; font-size: 13px; font-weight: 900; font-family: 'Helvetica'; padding: 5px; padding-left: 8px; border: none; }"
		"QToolButton:hover { color: #943691; font-size: 13px; font-weight: 900; font-family: 'Helvetica'; border: none; padding-top: 5px; padding-bottom: 5px; }"
		"QToolButton:checked { color: #000000; font-size: 13px; font-weight: 900; font-family: 'Helvetica';  border: none; padding-top: 5px; padding-bottom: 5px; }"
		"QToolButton:pressed { color: #000000; font-size: 13px; font-weight: 900; font-family: 'Helvetica'; border: none; padding-top: 5px; padding-bottom: 5px; }"
		"#tabs { color: #000000; background-image:url(\":/images/rocket\"); background-repeat: no-repeat;  background-position: center; border: none; padding-top: 0px; padding-bottom: 0px; }");
...
void BitcoinGUI::setClientModel(ClientModel *clientModel)
{
	...
	if (clientModel)
	{
		...
		if (clientModel->isTestNet())
		{
			...
			if (trayIcon)
			{
				trayIcon->setToolTip(tr("Talium client") + QString(" ") + tr("[testnet]"));
...
void BitcoinGUI::createTrayIcon()
{
	...
	trayIcon->setToolTip(tr("Talium client"));
...
void BitcoinGUI::setNumConnections(int count)
{
	...
	labelConnectionsIcon->setToolTip(tr("%n active connection(s) to Talium network", "", count));
}
...
void BitcoinGUI::dropEvent(QDropEvent *event)
{
	if (event->mimeData()->hasUrls())
	{
		...
		if (nValidUrisFound)
			...
			notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid Talium address or malformed URI parameters."));
	}
