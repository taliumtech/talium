...
namespace {
    class CTaliumAddressVisitor : public boost::static_visitor<bool> {
    private:
        CTaliumAddress *addr;
    public:
        CTaliumAddressVisitor(CTaliumAddress *addrIn) : addr(addrIn) { }
...
bool CTaliumAddress::Set(const CKeyID &id) {
    SetData(Params().Base58Prefix(CChainParams::PUBKEY_ADDRESS), &id, 20);
    return true;
}

bool CTaliumAddress::Set(const CScriptID &id) {
    SetData(Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS), &id, 20);
    return true;
}

bool CTaliumAddress::Set(const CTxDestination &dest) {
    return boost::apply_visitor(CTaliumAddressVisitor(this), dest);
}

bool CTaliumAddress::IsValid() const {
...
CTxDestination CTaliumAddress::Get() const {
    if (!IsValid())
...
bool CTaliumAddress::GetKeyID(CKeyID &keyID) const {
...
bool CTaliumAddress::IsScript() const {
    return IsValid() && vchVersion == Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS);
}

void CTaliumSecret::SetKey(const CKey& vchSecret) {
...
CKey CTaliumSecret::GetKey() {
    CKey ret;
    ret.Set(&vchData[0], &vchData[32], vchData.size() > 32 && vchData[32] == 1);
    return ret;
}

bool CTaliumSecret::IsValid() const {
    bool fExpectedFormat = vchData.size() == 32 || (vchData.size() == 33 && vchData[32] == 1);
    bool fCorrectVersion = vchVersion == Params().Base58Prefix(CChainParams::SECRET_KEY);
    return fExpectedFormat && fCorrectVersion;
}

bool CTaliumSecret::SetString(const char* pszSecret) {
    return CBase58Data::SetString(pszSecret) && IsValid();
}

bool CTaliumSecret::SetString(const std::string& strSecret) {
