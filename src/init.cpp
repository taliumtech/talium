...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: Talium.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: Taliumd.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeTaliumamount=<n> " + _("Keep N Talium anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Talium is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Talium is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Talium version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "Talium server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Talium") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Talium to complete") << "\n";
    ...
    nAnonymizeTaliumAmount = GetArg("-anonymizeTaliumamount", 0);
    if(nAnonymizeTaliumAmount > 999999) nAnonymizeTaliumAmount = 999999;
    if(nAnonymizeTaliumAmount < 2) nAnonymizeTaliumAmount = 2;
    ...
    LogPrintf("Anonymize Talium Amount %d\n", nAnonymizeTaliumAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1TLM+1000 == (.1TLM+100)*10
       10TLM+10000 == (1TLM+1000)*10
    */
