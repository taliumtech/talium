...
int nAnonymizeTaliumAmount = 1000;
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Talium
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Talium
    // Mac: ~/Library/Application Support/Talium
    // Unix: ~/.Talium
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Talium";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "Talium.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "Taliumd.pid"));
